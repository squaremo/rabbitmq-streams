...
public class email_sender extends Server {
	...
	public final Server.ServerInputReader input = new Server.ServerInputReader() {
		...
		@Override
		public void handleBodyForTerminal(byte[] body, String key, long tag)
	...
	public email_sender(JSONObject config) throws IOException {
		super(config);
	...
	private class EmailDestination {
		...
		public void send(byte[] msg) throws MessagingException {
			...
			sendMail(host, username, password, "The subject, to be extracted from headers", new String(
					msg), to);
		}
