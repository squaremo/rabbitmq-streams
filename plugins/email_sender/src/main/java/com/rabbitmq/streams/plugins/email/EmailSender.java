    ...
    public void handleBodyForTerminal(byte[] body, String key, InputMessage ack) throws PluginException {
      ...
      if (null != dests) {
        try {
          ...
          notifier.notify(NotificationType.Unavailable, "Could not deliver email " + e.getMessage().trim());
